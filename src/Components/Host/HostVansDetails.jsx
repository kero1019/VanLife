...
export default function HostVansDetails() {
  ...
  return (
    <div className="bg-main-background p-5">
      <Link to="/VanLife/host/hostVans" className=" p-2 w-fit">
      ...
      <div className="bg-white p-5 rounded-xl">
        <div className="flex gap-4 items-center">
          <img src={vanData.imageUrl} alt="Photo" className="w-[15rem]"  />
          <div className="flex flex-col gap-8">
            <p>
            <Button type = {vanData.type}>{vanData.type}</Button>
            </p>
            <p className="font-bold text-2xl">{vanData.name}</p>
            <p className="flex items-center"><span className="font-bold text-[1.5rem]">${vanData.price}</span>/day</p>
          </div>
        </div>
      </div>
    </div>
